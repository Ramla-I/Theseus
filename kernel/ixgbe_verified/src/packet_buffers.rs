...
impl<const N: MTU> PacketBuffer<N> {
    ...
    /// # Note: The `size_in_bytes` should include the total length, payload + headers + checksum. 
    ...
    pub fn new(mut size_in_bytes: u16) -> Result<PacketBuffer<{MTU::Standard}>, &'static str> {
        if size_in_bytes >  MAX_STANDARD_ETHERNET_FRAME_LEN_IN_BYTES {
            return Err("Size of packet buffer is larger than MTU");
        } else if size_in_bytes < MIN_ETHERNET_FRAME_LEN_IN_BYTES{
            size_in_bytes = MIN_ETHERNET_FRAME_LEN_IN_BYTES;
        }
