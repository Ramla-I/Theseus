...
use nic_buffers_fast::PacketBuffer;
...
pub fn init_rx_queue<T: RxDescriptor, S:RxQueueRegisters>(num_desc: usize, rx_buffer_pool: &'static mpmc::Queue<PacketBuffer>, buffer_size: usize, rxq_regs: &mut S)
    -> Result<(BoxRefMut<MappedPages, [T]>, Vec<PacketBuffer>), &'static str> 
{    
    ...
    let mut rx_bufs_in_use: Vec<PacketBuffer> = Vec::with_capacity(num_desc);
