#![no_std]
//! This crate contains callbacks to create `TrustedChunk` objects and then `UnmappedFrames` objects from an `UnmappedFrames`.
...
/// The `TrustedChunk` is then used to create an `UnmappedFrames`.
/// 
